...
#Libraries for email verification
from django.contrib.sites.shortcuts import get_current_site
from django.template.loader import render_to_string
from django.utils.http import urlsafe_base64_encode, urlsafe_base64_decode
from django.utils.encoding import force_bytes
from django.contrib.auth.tokens import default_token_generator
from django.core.mail import EmailMessage # email ends
        ...
        if user is not None:
            try:
                ...
                if cart_item_exists:
                    ...
                    for productt in product_variation:
                        if productt in ex_var_list:
                            index = ex_var_list.index(productt)
                            item_id = id[index]
...
def forgotPassword(request):
    if request.method == 'POST':
        ...
        if Account.objects.filter(email=email).exists():
            ...
            print("UID:", uid)  # Check if this outputs a valid base64 string
            print("Token:", token)  # Check if this outputs a valid token
...
def resetpassword_validate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token): 
        ...
        messages.success(request, 'Please reset your passowrd') 
...
def my_orders(request):
    ...
    context = { # this we will used to send the order to the front-end
        'orders' : orders,
    }
